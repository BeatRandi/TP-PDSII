...
#include <cctype>      // Manipular caracteres
...
vector<string> Maquina_De_Busca::recuperar(vector<string> buscar) {
    ...
    // Normaliza as palavras de busca
    // Palavras repetidas não são readicionadas
    ...
    // Verificar se a palavra está presente em cada documento
    for (const auto &palavra : palavrasNormalizadas) {
        if (ocorrencia_.count(palavra) == 0) {
            // Palavra não encontrada em nenhum documento, retornar lista vazia
            return documentosEncontrados;
        }
    }

    // Encontrar documentos que contêm todas as palavras
    ...
    // Ordenar documentos por número de hits e lexicograficamente
